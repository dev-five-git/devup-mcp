...
fn pinned_plugin_corpus_is_complete_and_self_consistent() {
    ...
    let summary = support::validate_corpus(&root).unwrap_or_else(|violations| {
        panic!("compat corpus violations:\n{}", violations.join("\n"))
    });
...
fn coverage_registry_maps_every_inventory_entry_to_real_evidence() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/devup-figma-plugin");
    let summary = support::validate_coverage_registry(&root).unwrap_or_else(|violations| {
        panic!("coverage registry violations:\n{}", violations.join("\n"))
    });
    ...
    assert_eq!(
        ...
        "every upstream inventory entry must have executable evidence or an explicit scope classification."
    );
}
