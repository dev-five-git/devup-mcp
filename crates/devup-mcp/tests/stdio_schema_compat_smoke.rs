...
fn tools_list_over_raw_stdio_has_no_boolean_schemas_and_object_output_types() -> anyhow::Result<()>
{
    ...
    assert_eq!(
        tools.len(),
        10,
        "expected all 10 devup-mcp tools (4 devup_figma_* + devup_skills + devup_project_context + devup_ui_validate + devup_stack_diff + devup_visual_compare + devup_feature_trace) to be listed: {tools:?}"
    );
