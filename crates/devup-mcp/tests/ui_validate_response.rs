...
-> anyhow::Result<()> {
    for tsx in [
        ...
        assert_eq!(output["recoveryState"], "unrecoverable");
...
{
    let output = validate(json!({"tsx":r##"<><Text>한글</Text><Box bg="#3366ff" p="16px" /></>"##,"strict":true,"projectRoot":fixture_project_root()})).await?;
    assert_eq!(output["ok"], false);
    ...
    assert_eq!(arguments["projectRoot"], fixture_project_root());
    assert_eq!(
        arguments["tsx"],
